...
const Payment = () => {
  ...
  const [open, setOpen] = useState(false);
  const [select, setSelect] = useState(1); // 1 = card, 2 = paypal, 3 = cod
  const { user } = useSelector((state) => state.user);
  const navigate = useNavigate();
  const stripe = useStripe();
  const elements = useElements();
    ...
    <div className="w-full flex flex-col items-center py-8">
      <div className="w-[90%] 1000px:w-[70%] block 800px:flex">
            ...
            {select === 1 && (
              <form className="w-full" onSubmit={paymentHandler}>
                <div className="w-full flex pb-3 gap-2">
                  <div className="w-[50%]">
                    <label className="block pb-2">Name on Card</label>
                    <input value={user?.name} readOnly className={`${styles.input}`} />
                  </div>
                  <div className="w-[50%]">
                    <label className="block pb-2">Exp Date</label>
                    <CardExpiryElement className={`${styles.input}`} />
                ...
                {open && (
                  <div className="fixed top-0 left-0 w-full h-screen bg-[#00000039] flex items-center justify-center z-[99999]">
                    <div className="bg-white rounded-[5px] p-8 w-[90%] 800px:w-[40%] shadow relative overflow-y-scroll max-h-[80vh]">
                      ...
                      <PayPalScriptProvider options={{ "client-id": "AXRhO4..." }}>
        ...
        <div className="w-full 800px:w-[35%] 800px:mt-0 mt-8">
          <div className="w-full bg-white rounded-md p-5 pb-8">
            <div className="flex justify-between">
              <h3 className="text-[16px] text-[#000000a4]">Subtotal:</h3>
              <h5 className="text-[18px] font-[600]">${orderData?.subTotalPrice}</h5>
            </div>
            <div className="flex justify-between mt-2">
              <h3 className="text-[16px] text-[#000000a4]">Shipping:</h3>
              <h5 className="text-[18px] font-[600]">${orderData?.shipping?.toFixed(2)}</h5>
            </div>
            <div className="flex justify-between mt-2 border-b pb-3">
              <h3 className="text-[16px] text-[#000000a4]">Discount:</h3>
              <h5 className="text-[18px] font-[600]">
                {orderData?.discountPrice ? `$${orderData.discountPrice}` : "-"}
              </h5>
            </div>
