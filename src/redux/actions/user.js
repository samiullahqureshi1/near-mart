...
export const loadUser = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `https://near-backend.vercel.app/api/v2/user/getuser`,
      { headers: authHeader() }
    );
    ...
    const errMsg = error?.response?.data?.message || error.message || "Something went wrong";
    console.error("[loadUser] API call failed:", errMsg);
    dispatch({ type: "LoadUserFail", payload: errMsg });
  }
};
...
export const loadSeller = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `https://near-backend.vercel.app/api/v2/shop/getSeller`,
      { headers: authHeader() }
    );
...
export const getAllUsers = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `https://near-backend.vercel.app/api/v2/user/admin-all-users`,
      { headers: authHeader() }
    );
...
export const updateFullUserInfo = (userId, updatedUserData) => async (dispatch) => {
  try {
    ...
    dispatch({ type: "updateFullUserSuccess", payload: data.user });
  } catch (error) {
    dispatch({
      type: "updateFullUserFailed",
      payload: error.response?.data?.message || "Update failed",
